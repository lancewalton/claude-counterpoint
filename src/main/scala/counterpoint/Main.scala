...
@main def main(): Unit = 
  ...
  println("\nShowing the application of melodic rules:")
  val rules = new MelodicRules()
  
  println("1. Within octave rule:")
  println(s"  C4 to C5 is within octave: ${rules.isWithinOctaveRule(Note.C4, Note.C5)}")
  println(s"  C4 to D5 is within octave: ${rules.isWithinOctaveRule(Note.C4, Note.D5)}")
  
  println("2. Not a seventh rule:")
  println(s"  C4 to B4 is a seventh interval: ${Note.C4.intervalSize(Note.B4)}")
  println(s"  Rule passes? ${rules.notASeventhRule(Note.C4, Note.B4)}")
